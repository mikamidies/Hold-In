  ...
  >
    <div class="my_container flex items-center justify-between">
      <div class="flex items-center gap-5">
        <div
          @click="burger = true"
          class="block lg:hidden px-16 py-14 rounded-8 bg-purple cursor-pointer"
        >
      ...
      <div class="hidden lg:flex gap-20 xl:gap-30 2xl:gap-40">
        <NuxtLink
          to="/"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_1") }}</NuxtLink
        >
        <NuxtLink
          to="/about"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_2") }}</NuxtLink
        >
        <NuxtLink
          to="/holdings"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_3") }}</NuxtLink
        >
        <NuxtLink
          to="/gallery"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_4") }}</NuxtLink
        >
        <NuxtLink
          to="/news"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_5") }}</NuxtLink
        >
        <NuxtLink
          to="/contact"
          class="text-18 font-interfaces font-medium leading-150"
          :class="`text-${color}`"
          >{{ $t("page_name_6") }}</NuxtLink
        >
      </div>

      <div class="relative px-15">
        <div
          class="relative flex items-center gap-4 z-10 cursor-pointer"
          @click="lang = !lang"
        >
          <p
            class="font-inter font-medium leading-150"
            :class="`text-${color}`"
          >
            {{ lang_value }}
          </p>
        ...
        <div
          class="language_box flex flex-col gap-8 absolute top-0 left-0 px-15 py-10 border border-grey_40 rounded-4"
          :class="[`bg-${bg_color}`, lang ? 'active' : 'not_active']"
        >
          <p
            @click="changeLang('ru', `Русский`)"
            :class="`text-${color} cursor-pointer leading-110`"
          >
            Русский
          </p>
          <p
    ...
    <!-- Burger -->
    <div
      class="burger_div w-full min-h-screen fixed lg:hidden overflow-hidden top-0 right-0 z-40"
      :class="burger ? 'active' : ''"
    >
      <div
        @click="burger = false"
        class="w-full z-40 min-h-screen absolute top-0 left-0"
      ></div>
      <div
        class="w-80% burger_bg bg-Rich-Black z-50 min-h-screen absolute top-0 left-0 px-16 py-50"
      >
        <div class="flex items-center gap-20 mb-76">
          <img
            ...
            alt=""
          />
          <p class="w-full text-20 text-center font-medium pr-44 text-white">
            Меню
          </p>
        </div>
        ...
        <div class="flex flex-col gap-16">
          <NuxtLink
            @click="burger = false"
            class="font-interfaces text-18 font-semibold text-white"
            to="/about"
            >{{ $t("page_name_2") }}</NuxtLink
          >
          <NuxtLink
    ...
    window.addEventListener("scroll", () => {
      ...
      if (scrollTop > lastScrollTop) {
        ...
        if (window.pageYOffset == 0) {
          ...
          navbar.style.backgroundColor = `${sessionStorage.getItem(
            "bg_color"
          )}`;
